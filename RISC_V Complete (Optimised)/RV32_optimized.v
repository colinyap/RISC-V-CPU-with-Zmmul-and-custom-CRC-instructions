...
//==========================================================================
// M1: fixed-window radix-4 Booth, derived from signed radix-4 recurrence.
// Same ports, enabled-edge latency, sticky ready and restart behavior as original.
module RadixBooth (inputA, inputB, result, clk, reset, en, start, ready, busy, start_posedge);
 ...
 reg [5:0] i;
 reg start_prev;
 wire [33:0] m_ext = {{2{M[31]}}, M};
 reg [33:0] addend;
 always @(*) begin
   case ({lower[1:0],previous_bit})
     3'b001,3'b010: addend=m_ext;
     3'b011: addend={m_ext[32:0],1'b0};
     3'b100: addend=-{m_ext[32:0],1'b0};
     3'b101,3'b110: addend=-m_ext;
     default: addend=34'd0;
   endcase
 end
 wire [33:0] sum = upper + addend;
 always @(*) start_posedge = (start && !start_prev) ? 1 : 0;
 always @(posedge clk) begin
   if (!reset) begin
     M<=0; upper<=0; lower<=0; previous_bit<=0; i<=0;
     result<=0; ready<=0; busy<=0; start_prev<=0;
   end else if (en) begin
     start_prev<=start;
     if (start && !start_prev) begin
       M<=inputA; upper<=0; lower<=inputB; previous_bit<=0;
       i<=0; ready<=0; busy<=1;
     end else if (busy) begin
       if (i==6'd32) begin
         // Identity addition preserves original full-width arithmetic X propagation.
         result<=({upper[31:0],lower} + 64'd0); ready<=1; busy<=0;
       end else begin
         // Arithmetic shift of {sum,lower,previous_bit} by two bits.
         upper<={{2{sum[33]}},sum[33:2]};
         lower<={sum[1:0],lower[31:2]};
         previous_bit<=lower[1];
         i<=i+6'd2;
       end
     end
   end
...
// W1P: carry-save high correction with original procedural selection and
// full-width arithmetic X propagation. No synthesis/simulation split.
module MultiplierWrapper (input [63:0] raw_input,
 output reg [31:0] out, input [2:0] funct3,
 input [31:0] rs1, input [31:0] rs2);
 reg [31:0] a,b;
 reg correction;
 always @(*) begin
   a=0; b=0; correction=0;
   case(funct3)
     3'd2: if(rs2[31]) begin a=rs1; correction=1; end
     3'd3: begin
       if(rs1[31] && rs2[31]) begin a=rs1; b=rs2; correction=1; end
       else if(rs1[31]) begin b=rs2; correction=1; end
       else if(rs2[31]) begin a=rs1; correction=1; end
     end
     default: begin end
   endcase
 end
 wire [31:0] h=raw_input[63:32];
 wire [31:0] sum=h^a^b;
 wire [31:0] carry=((h&a)|(h&b)|(a&b))<<1;
 ...
 wire [63:0] corrected={sum,raw_input[31:0]}+{carry,32'd0};
 always @(*) begin
   case(funct3)
     3'd0: out=raw_input[31:0];
     3'd1: out=raw_input[63:32];
     3'd2,3'd3: out=correction ? corrected[63:32] : raw_input[63:32];
     default: out=32'd0;
   endcase
 end
endmodule
